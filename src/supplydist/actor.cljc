(ns supplydist.actor
  "SupplyDistributionManagersActor — the ISCO-08 1324 community supplydist actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one supplydist operation request
  ...
  The unconditional invariant: the SupplyDistributionManagersAdvisor can never
  directly commit a record the SupplyDistributionManagersGovernor refuses —
...
(defn build-graph
  "Build a compiled SupplyDistributionManagersActor graph. `store` implements
  `supplydist.store/Store`. `advisor` implements
  `supplydist.advisor/Advisor` (defaults to `mock-advisor`).
